...
/*   Updated: 2023/02/13 20:00:07 by cpapot           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t	ft_findfor_fd(int fd, char const *format, int index, va_list variadic)
{
	ssize_t	len;
...
ssize_t	ft_callformat_fd(int fd, char const *format, va_list variadic)
{
	...
	while (format[index] != '\0')
	{
		...
		{
			tmp = ft_findfor_fd(fd, format, index, variadic);
			index = index + 2;
		}
		if (tmp == -1)
			return (-1);
